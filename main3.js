        ...
        for (let circle of circles) {
            if (circle !== this) {
                ...
                if (distance < (this.radius + circle.radius)) {
                    // Calculamos el ángulo de rebote
                    let angle = Math.atan2(circle.posY - this.posY, circle.posX - this.posX);

                    // Calculamos las velocidades tangenciales y normales
                    let tangentSpeedX1 = this.dy * Math.sin(angle) + this.dx * Math.cos(angle);
                    let tangentSpeedY1 = this.dy * Math.cos(angle) - this.dx * Math.sin(angle);
                    let tangentSpeedX2 = circle.dy * Math.sin(angle) + circle.dx * Math.cos(angle);
                    let tangentSpeedY2 = circle.dy * Math.cos(angle) - circle.dx * Math.sin(angle);

                    // Calculamos las nuevas velocidades tangenciales después de la colisión
                    let finalTangentSpeedX1 = (tangentSpeedX1 * (this.radius - circle.radius) + 2 * circle.radius * tangentSpeedX2) / (this.radius + circle.radius);
                    let finalTangentSpeedX2 = (tangentSpeedX2 * (circle.radius - this.radius) + 2 * this.radius * tangentSpeedX1) / (this.radius + circle.radius);

                    // Convertimos las velocidades tangenciales de nuevo a coordenadas cartesianas
                    this.dx = Math.cos(angle) * finalTangentSpeedX1 + Math.sin(angle) * tangentSpeedY1;
                    this.dy = Math.sin(angle) * finalTangentSpeedX1 - Math.cos(angle) * tangentSpeedY1;
                    circle.dx = Math.cos(angle) * finalTangentSpeedX2 + Math.sin(angle) * tangentSpeedY2;
                    circle.dy = Math.sin(angle) * finalTangentSpeedX2 - Math.cos(angle) * tangentSpeedY2;

                    // Mueve los círculos para evitar que se superpongan
                    this.posX += this.dx / 2;
                    this.posY += this.dy / 2;
                    circle.posX += circle.dx / 2;
                    circle.posY += circle.dy / 2;

                    // Indica que hubo una colisión
                    isColliding = true;
                }
            }
        }
